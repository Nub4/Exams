...
int ft_intersect(t_all *a, float x, float y)
{
    float dist;

    dist = sqrtf(powf(x - a->x, 2) + powf(y - a->y, 2));
    if (dist <= a->rad)
    {
        if (a->rad - dist < 1.00000000)
            return (2);
        return (1);
    }
    return (0);
...
void    ft_save(t_all *a)
{
    ...
    while (y < a->b_height)
    {
        ...
        while (x < a->b_width)
        {
            hit = ft_intersect(a, (float)x, (float)y);
            if ((hit == 2 && a->type == 'c') || (hit && a->type == 'C'))
...
int ft_draw(t_all *a)
{
    ...
    while ((count = fscanf(a->f, "%c %f %f %f %c\n", &a->type, &a->x, &a->y, &a->rad, &a->c)) == 5)
    {
        if (ft_check(a) == 1)
            return (1);
        ft_save(a);
